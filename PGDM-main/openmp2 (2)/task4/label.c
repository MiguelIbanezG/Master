...
tLabel Label_Binary_Image(tGrayImage *image, tLabelArray *labels)
{
	...
	//#pragma omp parallel for private(col, pixel) schedule(static)
	for (row = 0; row < image->rows; row++)
		...
		for (col = 0; col < image->cols; col++)
		{
			...
			if (pixel == BLACK)
				Write_Label(labels, row, col, 0);
			else if (pixel == WHITE)
				Write_Label(labels, row, col, UNLABELED);
	...
	// printf("Ini OK\n");
	//#pragma omp parallel for private(col) reduction(+:curlabel)
	for (row = 0; row < image->rows; row++)
		for (col = 0; col < image->cols; col++)
	...
	// #pragma omp parallel for private(col, label)
	for (row = 0; row < image->rows; row++)
		for (col = 0; col < image->cols; col++)
		{
