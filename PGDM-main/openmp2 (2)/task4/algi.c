...
void Add_Color_Images(tColorImage *src1, tColorImage *src2, tColorImage *dst)
{
	...
	//#pragma omp parallel for private(c, pixel1, pixel2, i, tmp, pixel)
...
void Binarize_Gray_Image(tGrayImage *src, tGrayPixel threshold, tGrayImage *dst)
{
	...
	//#pragma omp parallel for private(c, pixel) schedule(dynamic)
...
void Or_Gray_Images(tGrayImage *src1, tGrayImage *src2, tGrayImage *dst)
{
	...
	// #pragma omp parallel for private(c, pixel1, pixel2) schedule(dynamic)
	for (r = 0; r < src1->rows; r++)
		for (c = 0; c < src1->cols; c++)
		{
...
int Otsu_Binarize_Gray_Image(tGrayImage *src, tGrayImage *dst)
{
	...
	acum = 0;
	// #pragma omp parallel for schedule(static) reduction(+:acum)
	for (i = 0; i < 256; i++)
		acum += histo[i];
	
	// #pragma omp parallel for schedule(static)
	for (i = 0; i < 256; i++)
		nhisto[i] = (float)histo[i] / acum;
...
void Color_Image_to_Gray(tColorImage *src, tGrayImage *dst)
{
	...
	#pragma omp parallel for private(c, i, max, pixel, gpixel) schedule(dynamic)
	for (r = 0; r < src->rows; r++)
		for (c = 0; c < src->cols; c++)
		{
...
void Graylevel_Histogram(tGrayImage *src, int graylevels, int *histo)
{
	...
	//#pragma omp parallel for private(c, pixel) reduction(+:histo[pixel]) schedule(dynamic)
	for (r = 0; r < src->rows; r++)
		for (c = 0; c < src->cols; c++)
		{
			BMP_Read_Gray_Pixel(src, r, c, &pixel);
...
void Conditional_Graylevel_Histogram(tGrayImage *src, tGrayImage *cnd, int graylevels, int *histo)
{
	...
	// #pragma omp parallel for private(c, pixel, flag) reduction(+:histo[pixel]) schedule(dynamic)
	for (r = 0; r < src->rows; r++)
		for (c = 0; c < src->cols; c++)
		{
			BMP_Read_Gray_Pixel(src, r, c, &pixel);
...
void Sobel_Filter(tGrayImage *src, tGrayImage *dst)
{
	...
	// #pragma omp parallel for private(r, c, tmp1, tmp2, max, min) schedule(dynamic)
	...
	// #pragma omp parallel for private(r, c, tmp, pixel) schedule(dynamic)
	for (r = 1; r < src->rows - 1; r++)
		for (c = 1; c < src->cols - 1; c++)
		{
...
void Binary_Dilation(tGrayImage *src, tGrayImage *dst)
{
	...
	//#pragma omp parallel for private(c, pixel, flag, r1, r2, c1, c2, rr) schedule(dynamic)
	for (r = 0; r < src->rows; r++)
		for (c = 0; c < src->cols; c++)
		{
			BMP_Read_Gray_Pixel(src, r, c, &pixel);
			if (pixel == 0)
			{
				...
				if (flag)
					BMP_Write_Gray_Pixel(dst, r, c, 255);
			}
		}
}
...
void Binary_Erosion(tGrayImage *src, tGrayImage *dst)
{
	...
	// #pragma omp parallel for private(r, c, pixel) schedule(dynamic)
	for (r = 0; r < src->rows; r++)
		for (c = 0; c < src->cols; c++)
		{
			BMP_Read_Gray_Pixel(src, r, c, &pixel);
	...
	// #pragma omp parallel for private(r, c, pixel, flag, r1, r2, c1, c2, rr, cc) schedule(dynamic)
	for (r = 0; r < src->rows; r++)
		for (c = 0; c < src->cols; c++)
		{
			BMP_Read_Gray_Pixel(src, r, c, &pixel);
			if (pixel == 255)
			{
				...
				if (flag)
					BMP_Write_Gray_Pixel(dst, r, c, 0);
			}
		}
}
...
main(int argc, char **argv)
{
	...
	if (argc == 4)
		ThresholdYellow = atoi(argv[3]);
	else
		ThresholdYellow = THRESHOLD;
	...
	//#pragma omp parallel for private(r, c, pixel, label) reduction(+:alga[label].sumgroc) schedule(dynamic)
	for (r = 0; r < Labels.rows; r++)
		for (c = 0; c < Labels.cols; c++)
		{
			...
			if (label > 0)
			{
				label--;
				alga[label].area++;
				if (r < alga[label].rowmin)
					alga[label].rowmin = r;
				if (r > alga[label].rowmax)
					alga[label].rowmax = r;
				if (c < alga[label].colmin)
					alga[label].colmin = c;
				if (c > alga[label].colmax)
					alga[label].colmax = c;
				...
				if (pixel == 255)
				{
					...
					if (pixel > alga[label].maxgroc)
						alga[label].maxgroc = pixel;
					if (pixel < alga[label].mingroc)
						alga[label].mingroc = pixel;
	...
	//#pragma omp parallel for private(ratio, mitja, sigma, colpixel, r, c) schedule(dynamic)
	for (i = 0; i < n; i++)
	{
